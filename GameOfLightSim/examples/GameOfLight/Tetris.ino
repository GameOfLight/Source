...
uint8_t tetris_drawPiece(uint8_t x, uint8_t y, uint8_t piece, uint8_t rotation) {
  int8_t xmod, ymod;
  uint8_t colour;
...
uint8_t tetris_isWithinSpawnBox(uint8_t p, struct piece* active) {
...
void tetris_drawScore(uint8_t p, uint16_t score) {
  ...
  frame.gotoXY(tetris_bounds[p][0]+2, 1);
  frame.clear(17);
  if (score <= 999) {
    char buff[4];
    ...
    for (int i = 0; i < playerCnt; i++) {
      if ((player_state[i] != TETRIS_DEAD) && (abs(last_time[i] - currTime) > (768 - (level[i]/10)*38))) {
        ...
        if (player_state[i] == TETRIS_LOCKED) {
          //Current piece locked in place. Check and clear any applicable lines
          ...
          if (player_state[i] == TETRIS_NORMAL || player_state[i] == TETRIS_LOCK_DELAY) {
            ...
            if (tetris_move(i, &pieces[i], SOUTH)) {
              ...
              if (player_state[i] == TETRIS_LOCK_DELAY) {
                ...
                if (tetris_isWithinSpawnBox(i, &pieces[i])) {
